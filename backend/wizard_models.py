...
class WelcomeForm(BaseModel):
    ...
    industry: Industry = Field(..., description="User's industry")
...
class CompanyForm(BaseModel):
    ...
    number_of_employees: EmployeeRange = Field(..., description="Number of employees range")
    goals: str = Field(..., min_length=10, max_length=500, description="Company goals and objectives")
    subscribe_to_updates: bool = Field(default=False, description="Subscribe to product updates and tips")
...
class PersonalizationForm(BaseModel):
    ...
    # Form fields
    vibe: VibeType = Field(..., description="User's personality type")
    favorite_color: str = Field(..., regex=r"^#[0-9A-Fa-f]{6}$", description="Favorite color in hex format")
...
class OnboardingWizard(BaseModel):
    ...
    # Forms in order
    forms: list[BaseModel] = [WelcomeForm, CompanyForm, PersonalizationForm, SummaryForm]
...
class FormFieldConfig(BaseModel):
    ...
    welcome_fields: list[FormField] = [
        TextFormField(
            name="full_name", label="Full Name *", placeholder="Enter your full name", min_length=2, max_length=50
        ),
        EmailFormField(name="email", label="Email Address *", placeholder="Enter your email address"),
        PasswordFormField(name="password", label="Password *", placeholder="Create a secure password"),
        SelectFormField(
            name="industry",
            label="Industry *",
    ...
    company_fields: list[FormField] = [
        ...
        CheckboxFormField(name="subscribe_to_updates", label="Subscribe to product updates and tips", required=False),
    ]
